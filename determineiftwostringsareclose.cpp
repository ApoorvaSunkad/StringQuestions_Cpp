/*
wo strings are considered close if you can attain one from the other using the following 
operations:
...
//T.C = O(NLOGN) + O(N+N) => O(26LOG(26)) + O(2N) =>  final T.C = O(N)
//S.C = O(1) //freq array takes O(1) S.C as the array size is fixed.
