...
is encountered or end of the string is reached. If no digits are present, 
return 0.
...
int myAtoi(char *s) {
        // Your code here
        long long int ans = 0;  // Use long long int to handle larger numbers
        int len = strlen(s);
        int sign = 1;  // To track the sign of the number
        int i = 0;

    // Ignore leading whitespaces if there are any (optional)
    while (s[i] == ' ') {
        i++;
    }

    // Check for a minus sign or plus sign at the start
        if (s[i] == '-') {
        ...
        for (; i < len; i++) {
            ...
            if (s[i] >= '0' && s[i] <= '9') {
                ...
                // Check for overflow and return max/min values if overflow happens
                if (ans * sign > INT_MAX) return INT_MAX;
                if (ans * sign < INT_MIN) return INT_MIN;
            } else {
                // Stop processing once a non-digit character is encountered
                break;
            }
        }
